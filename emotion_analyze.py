...
def extract_event_url(result_end_file, delimiter):
    #抽取主要事件的url，用于匹配与事件相对应的评论，丢弃与事件不相对应的评论
    event = sys.stdin if result_end_file is None else open(result_end_file, 'rb')
    event_url_list = []
    for event in event.readlines():
        event_url = event.strip().split(delimiter, 1)[0]
        if event_url not in event_url_list:
...
def main(**kw):
    ...
    event_url_list = extract_event_url(result_end_path, delimiter)
    comment_url_list = []
    for comment in read_input(stdin, delimiter):
        ...
        if new_url not in event_url_list:
            continue
        if new_url not in comment_url_list:
            comment_url_list.append(new_url)
    ...
    comment_empty_event_url = list(set(event_url_list) - set(comment_url_list))
    for empty_url in comment_empty_event_url:
        print empty_url#打印评论为空的事件url
